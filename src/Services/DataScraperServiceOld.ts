...
//     let date = moment().format("DD-MM-YYYY__HH:mm:ss").replace(" ", "__").replace(":", "-").replace(":", "-");
...
//       EmailService.SendErrorMail(err, "Refresh Data Section", date, restoreSuccessful);
...
//     try {
//       fs.appendFileSync(path.join(__dirname, "..", "Data", "liveData.json"), JSON.stringify(newData));
//     } catch (err) {
//       console.error(err);
//       this.RestoreData(date);
//       throw new LiveDataWriteError("Failed to create new live data! Reverting...");
//     }
...
//       try {
//         fs.truncateSync(path.join(__dirname, "..", "Data", "validLiveData.json"));
//       } catch (err) {
//         // this.RestoreData(date);
//         console.error(err);
//         throw new LiveDataWriteError("Failed to create new live data! Reverting...");
...
// export default new DataScraperServiceOld();
